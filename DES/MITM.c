...
void print_table(uint64_t iterations, uint8_t *table);
...
int main (void)
{
    ...
    while (cnt < iterations_k1 && !aurkitua){
        des(DECRYPTION, c1, p4, key1);
        j = search_in_blocks(table, iterations_k2, p4);
        if (j != -1) {
            aurkitua = 1;
            key2[0] = k_index[j] & 0xff;
            break;
    ...
    if (aurkitua){
        print_key(key1, 1);
        print_key(key2, 2);
        twodes(DECRYPTION, p, c1, key1, key2);
        print_plaintext(p);
        twodes(DECRYPTION, p, c2, key1, key2);
        print_plaintext(p);
        twodes(DECRYPTION, p, c3, key1, key2);
        print_plaintext(p);
        twodes(DECRYPTION, p, c4, key1, key2);
        print_plaintext(p);

    } else {
        printf("Ez da aurkitu\n");
    }
...
void print_table(uint64_t iterations, uint8_t *table){
    ...
    for (i=0; i<iterations; i++){
        for (j=0; j<BLOCK_SIZE; j++){
            printf("%x ", table[i*BLOCK_SIZE+j] & 0xff);
        }
        printf("\n");
    }
}
